...
<template>
  <v-container fill-height
    ...
    <div class="controls">
      ...
      <div class="t-sen-controller control-field"
        ...
        <div class="pa-3">
          ...
          <v-slider v-model="ui.tsne.perplexity"
          ...
          <v-slider v-model="ui.tsne.earlyExaggeration"
                    class="align-center"
                    :max="100"
                    :min="1"
                    hide-details>
          ...
          <v-slider v-model="ui.tsne.learningRate"
                    class="align-center"
                    :max="1000"
                    :min="100"
                    hide-details>
          ...
          <v-slider v-model="ui.tsne.iteration"
                    class="align-center"
                    :max="500"
                    step="10"
                    :min="100"
      ...
      <div class="line-controller control-field"
           :selected="ui.tab.activate ==='line'">
        <div class="pa-3">
          <div class="px-2">
            <b>Link 설정</b>
          </div>
          <div class="px-2 mb-2 caption">노드들의 연결 설정</div>
          <v-layout class="px-2"
                    align-center>
            <v-flex style="width:80%">
              <div style="padding-bottom : 8px;">
                Edge Status: <b>{{ui.edges.enableEdges ? 'Enabled' : 'Disabled'}}</b>
              </div>
            </v-flex>
            <v-flex>
              <v-switch v-model="ui.edges.enableEdges"
                        inset></v-switch>
            </v-flex>

          </v-layout>
          <div class="selector-header">Weight</div>
          <v-range-slider class="align-center ma-0"
                          v-model="ui.edges.weight"
                          :max="1.0"
                          :min="0.0"
                          step="0.01"
                          hide-details>
            <template v-slot:prepend>
              <div class="number-guide">{{ui.edges.weight[0]}}</div>
            </template>
            <template v-slot:append>
              <div class="number-guide">{{ui.edges.weight[1]}}</div>
            </template>
          </v-range-slider>
        </div>
        <div class="px-2">
          <div class="selector-header">Opacity</div>
          <v-slider v-model="ui.edges.opacity"
                    class="align-center"
                    :max="1"
                    step="0.01"
                    :min="0"
                    hide-details>
            <template v-slot:append>
              <div class="number-guide">{{ui.edges.opacity}}</div>
            </template>
          </v-slider>
        </div>
        <v-divider></v-divider>
    ...
    <div class="legend"
         v-show="Object.keys(palette).length !== 0">
      <div v-for="(value, key) in palette"
           :key="key"
           class="legend-item">
        <div class="legend-color"
             v-bind:style="{backgroundColor: value}"></div>
